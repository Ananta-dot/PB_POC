#!/usr/bin/env python3
# PatternBoost MISR search w/ seed injection and interior-only clique points
# - prefers MPS (Apple Silicon) over CUDA
# - can inject your n=14 1.5-gap seed(s) from a .pkl
# - has an --interior_only toggle to place clique points at cell centers
# - exposes --neighbor_k on CLI
...
# -------- Device selection: prefer MPS (Apple) over CUDA --------
def get_device():
...
# -------- Helpers for two-sequence span instances --------
...
# -------- Seeded motif pool (structured starts) --------
...
def grid_points_edges(rects: List[Rect]) -> List[Tuple[float,float]]:
...
def grid_points_cell_centers(rects: List[Rect]) -> List[Tuple[float,float]]:
    xs = sorted({x for r in rects for x in (r[0][0], r[0][1])})
    ys = sorted({y for r in rects for y in (r[1][0], r[1][1])})
...
def solve_lp_ilp(rects: List[Rect],
                 grb_threads: int = 0,
                 interior_only: bool = False) -> Tuple[float, float]:
    pts = grid_points_cell_centers(rects) if interior_only else grid_points_edges(rects)
...
def score_ratio(H: Seq, V: Seq,
                ...
                beta_ilp: float = 0.0,
                grb_threads: int = 0,
                interior_only: bool = False) -> Tuple[float,float,float,float]:
    """Returns (lp, ilp, ratio, blended) where blended guides local search."""
    rects = build_rects(H,V)
    lp, ilp = solve_lp_ilp(rects, grb_threads=grb_threads, interior_only=interior_only)
...
def load_seeds_from_pkl(seed_pkl: str, n: int, top_k: int = 4) -> List[Instance]:
    """Read (ratio,H,V) tuples and return up to top_k (H,V) with max(H)==n."""
    try:
...
def run_patternboost(
    ...
    n = n_start
    best_overall = 0.0
    seeds_list = seeded_pool(n, rng, seeds_per_round)

    # Inject strong starting seeds
    if seed_pkl:
        injected = load_seeds_from_pkl(seed_pkl, n, top_k=seed_top_k)
        if injected:
            seeds_list = injected + seeds_list[:max(0, seeds_per_round - len(injected))]
            print(f"[seed_inject] loaded {len(injected)} seeds from {seed_pkl} for n={n}")

    while n <= n_target:
        n_dir = os.path.join(run_dir, f"n{n:02d}")
        print(f"\n=== SIZE n={n} ({len(seeds_list)} seeds) ===")
        for r in range(rounds_per_n):
            # 1) Local search
            for (H,V) in seeds_list:
            ...
            # 3) New seeds: recombine + transformer + motifs + jitter (+ keep one injected)
            ...
            # keep one best injected (stability)
            if seed_pkl:
                injected = load_seeds_from_pkl(seed_pkl, n, top_k=1)
                if injected:
                    new_seeds[0] = injected[0]

            seeds_list = new_seeds
        ...
        if elites:
            ...
            for _, h, v in elites[:min(96, len(elites))]:
                ...
                lifted.append((h2,v2))
            seeds_list = lifted + seeds_list[:max(0, seeds_per_round - len(lifted))]
            n = n_next
        else:
            seeds_list = seeded_pool(n, rng, seeds_per_round)
...
def main():
    ...
    ap.add_argument("--neighbor_k", type=int, default=96,
                    help="number of neighbor proposals per local-search step")
    ap.add_argument("--grb_threads", type=int, default=0)
    ap.add_argument("--lift_step", type=int, default=3)
    ap.add_argument("--interior_only", action="store_true",
                    help="place clique points at cell centers (ignores corner-only touches)")
    ap.add_argument("--seed_pkl", type=str, default="",
                    help="optional pkl with [(ratio,H,V), ...] to inject at n_start")
    ap.add_argument("--seed_top_k", type=int, default=4,
                    help="how many top n_start seeds to inject from --seed_pkl")
