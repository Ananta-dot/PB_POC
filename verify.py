...
# - Can match runner's clique set via --interior_only (cell centers) or default edge grid
...
def resolve_input_path(p: str) -> str:
    ...
    if os.path.isfile(p) and p.endswith(".pkl"):
        return p
    if os.path.isdir(p):
        # First, try LATEST.txt
        cand = os.path.join(p, "LATEST.txt")
        if os.path.isfile(cand):
            with open(cand, "r") as f:
                name = f.read().strip()
            # LATEST may contain a relative filename
            if not os.path.isabs(name):
                name = os.path.join(p, name)
            if os.path.isfile(name):
                return name
        # Otherwise, pick the newest *.pkl under this directory
        all_pkl = []
        ...
        all_pkl.sort(key=lambda f: os.path.getmtime(f), reverse=True)
        return all_pkl[0]
    raise FileNotFoundError(f"Path not found or not a .pkl/dir: {p}")
...
def parse_elites(obj: Any):
    ...
    if not isinstance(obj, (list, tuple)):
        raise ValueError("Pickle top-level is not a list/tuple.")
    ...
    for item in obj:
        ...
        if isinstance(item, (list, tuple)):
            ...
            if len(item) >= 3 and _is_int_list(item[1]) and _is_int_list(item[2]):
                ...
                # Try to discover (H,V) inside a longer tuple
                Hcands = [x for x in item if _is_int_list(x)]
                if len(Hcands) >= 2:
                    H, V = list(Hcands[0]), list(Hcands[1])
    ...
    if not out:
        raise ValueError("Could not parse any (H,V) pairs from pickle.")
    return out

# -------------------------
# Geometry & MISR models
# -------------------------
...
def _grid_points_cell_centers(rects: List[Rect]) -> List[Tuple[float,float]]:
    xs = sorted({x for r in rects for x in (r[0][0], r[0][1])})
    ys = sorted({y for r in rects for y in (r[1][0], r[1][1])})
    ...
    return [(xm, ym) for xm in xmid for ym in ymid]
...
def solve_lp_ilp_with_vals(rects: List[Rect],
                           grb_threads: int = 0,
                           interior_only: bool = False):
    pts = _grid_points_cell_centers(rects) if interior_only else _grid_points_edges(rects)
...
def plot_rects(rects: List[Rect], y_vals, title: str, outpng: Optional[str]):
    fig, ax = plt.subplots(figsize=(6,6))  # single chart, default colors
...
def main():
    ...
    pa.add_argument("--pkl", required=True,
                    help="Path to a .pkl OR a directory containing LATEST.txt/PKLs")
    pa.add_argument("--rank", type=int, default=1,
                    help="1-based elite rank to verify/plot (after re-scoring)")
    pa.add_argument("--threads", type=int, default=0,
                    help="Gurobi threads (0 = default)")
    pa.add_argument("--interior_only", action="store_true",
                    help="Use cell-center clique points (matches --interior_only runs)")
    pa.add_argument("--list", type=int, default=0,
                    help="If >0, list top-K by recomputed ratio before plotting rank")
    pa.add_argument("--no_plot", action="store_true",
                    help="Skip plotting (only print numbers)")
    pa.add_argument("--out", type=str, default="",
                    help="Optional output PNG path (default: auto)")
    args = pa.parse_args()

    path = resolve_input_path(args.pkl)
    print(f"[verify] using: {path}")

    with open(path, "rb") as f:
        raw = pickle.load(f)

    bag = parse_elites(raw)   # [(stored_ratio_or_None, H, V), ...]
    # Re-score under the selected clique mode so verify matches your choice.
    rescored = []
    for stored_ratio, H, V in bag:
        rects = build_rects(H, V)
        lp, ilp, x_vals, y_vals = solve_lp_ilp_with_vals(
            rects, grb_threads=args.threads, interior_only=args.interior_only
        )
        ratio = (lp/ilp) if ilp > 0 else float("nan")
        rescored.append((ratio, lp, ilp, H, V, x_vals, y_vals))

    # Sort by recomputed ratio (desc), break ties by LP then -ILP (bigger LP and smaller ILP is better)
    ...
    if args.list and args.list > 0:
        ...
        print(f"\n[top {K} (recomputed)]")
        for i in range(K):
            ratio, lp, ilp, H, V, _, _ = rescored[i]
            n = max(H) if H else 0
            print(f"#{i+1:2d}  n={n:2d}  LP={lp:.3f}  ILP={ilp:.3f}  ratio={ratio:.4f}  "
                  f"lenH={len(H)} lenV={len(V)}")

    # Select rank (1-based)
    idx = max(0, min(len(rescored)-1, args.rank - 1))
    ratio, lp, ilp, H, V, x_vals, y_vals = rescored[idx]
    n = max(H) if H else 0
    rects = build_rects(H, V)

    print(f"\n[rank {args.rank}] n={n}  LP={lp:.3f}  ILP={ilp:.3f}  ratio={ratio:.4f}")
    print("rect vars (i: x_lp, y_ilp):")
    for i,(xv,yv) in enumerate(zip(x_vals, y_vals), start=1):
        print(f"  {i:3d}: {xv:6.3f}, {yv}")

    if not args.no_plot:
        title = (f"Rectangles (rank {args.rank}, n={n})  "
                 f"LP={lp:.2f}  ILP={ilp:.2f}  ratio={ratio:.3f}  "
                 f"{'(interior)' if args.interior_only else '(edge-grid)'}")
