...
# -----------------------
# Types & global constants
# -----------------------
Seq = List[int]
Instance = Tuple[Seq, Seq]

PAD_ID = 0                 # reserved for padding only; we also use it as a BOS surrogate
MAX_N = 128                # safety ceiling
VOCAB_SIZE = MAX_N + 1     # 0..MAX_N ; labels are 1..n

# -----------------------
# Seed loading / motifs
# -----------------------
def load_seeds_from_pkl(seed_pkl: str, n: int, top_k: int = 10) -> List[Instance]:
    """Read [(ratio,H,V), ...] or [(H,V), ...] and return up to top_k with max(H)==n."""
    try:
...
def seq_spans(seq: Seq) -> List[Tuple[int, int]]:
    """Return [(l_i, r_i)] for labels i=1..n (indices of two occurrences)."""
    first = {}
    spans = {}
...
# -----------------------
# Curriculum lifting (depth-weighted, original behavior)
# -----------------------
...
# -----------------------
# TinyGPT with n-conditioning (no special tokens)
# -----------------------
...
class TinyGPT(nn.Module):
    """Causal TransformerEncoder (decoder-only GPT style) with n-conditioning; vocab=PAD(0)+labels(1..MAX_N)."""
    def __init__(self, d=192, nhead=6, nlayers=3, dropout=0.1):
        super().__init__()
        self.label_embed = nn.Embedding(VOCAB_SIZE, d)         # includes PAD at 0
        self.n_embed     = nn.Embedding(MAX_N + 1, d)          # index by n
...
def seq_to_tokens(seq: Seq) -> List[int]:
    # labels already 1..n; PAD is 0 (not used in real tokens)
    return [int(i) for i in seq]

def tokens_to_seq(tokens: List[int]) -> Seq:
    return [int(t) for t in tokens if t != PAD_ID]

@dataclass
class Batch:
    tokens: torch.Tensor    # [B, L] long; values in [0..MAX_N], 0=PAD (also used as BOS surrogate)
    n_scalar: torch.Tensor  # [B] long
    targets: torch.Tensor   # [B, L] long; -100 on padded positions

def make_batch(elites: List[Tuple[float, Seq, Seq]], B: int, rng: random.Random) -> Batch:
    """
    We prepend a single PAD token to every training sequence (acts as BOS surrogate).
    Inputs:  tokens  = [PAD] + H(2n) + V(2n) + [PAD...]
    Targets: targets =  H(2n) + V(2n) + [PAD] + [ignore...]
    """
    tlist=[]; tglist=[]; ns=[]
    for _ in range(B):
        _, H, V = rng.choice(elites)
        n = max(H) if H else 1
        seq = seq_to_tokens(H) + seq_to_tokens(V)            # length 4n
        tok = [PAD_ID] + seq                                  # prepend PAD as BOS surrogate
        tgt = seq + [PAD_ID]                                  # next-token targets
    ...
    targets = torch.full((B, L), -100,   dtype=torch.long)  # -100 => ignore_index for CE
...
def sample_model(model: TinyGPT, n: int,
    ...
    """
    Generate exactly 2n labels for H, then exactly 2n labels for V (no SEP).
    We feed an initial PAD token as BOS surrogate (consistent with training).
    """
    model.eval()
    toks: List[int] = []  # generated label tokens so far (1..n)

    def step(mask_valid: List[bool]) -> int:
        # input to model is [PAD] + toks; model predicts distribution for the next token
        inp = torch.tensor([PAD_ID] + toks, dtype=torch.long, device=DEVICE).unsqueeze(0)
        nvec = torch.tensor([n], dtype=torch.long, device=DEVICE)
        logits = model(inp, nvec)[0, -1]  # [VOCAB_SIZE]
    ...
    while sum(counts) < 2*n and len(toks) < max_len:
        ...
        for i in range(1, n+1):
            if counts[i] < 2:
                mask[i] = True
        tok = step(mask)
        # ensure we never sample PAD (mask excludes it)
        tok = max(1, tok)
        toks.append(tok)
        counts[tok] += 1
...
def run_patternboost(
    ...
    while n <= n_target:
        ...
        # curriculum lift (depth-weighted)
        if elites:
            n_next = n + lift_step
            lifted=[]
