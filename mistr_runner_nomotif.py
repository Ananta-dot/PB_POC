...
BASE_VOCAB = 3            # first label id = BASE_VOCAB + (1-1) = 3
MAX_N = 128               # safety ceiling for embeddings
...
# ---------------------
# Seeded pool (now configurable)
# ---------------------
def seeded_pool(n: int, rng: random.Random, base_count: int,
                use_motif_seeds: bool = True,
                use_corner_combo: bool = True) -> List[Instance]:
    """Build an initial pool of (H,V) seeds."""
    seeds: List[Instance] = []
    if use_motif_seeds:
...
def lift_instance(H: Seq, V: Seq, n_new: int, rng: random.Random) -> Instance:
    """Insert pairs of new labels near congested areas (simple curriculum)."""
    assert n_new >= max(H)
    H2, V2 = H[:], V[:]
...
# ---------------------
# Local search (tabu + greedy + SA-lite)
# ---------------------
...
# ---------------------
# TinyGPT (token-level transformer proposer)
# ---------------------
...
class TinyGPT(nn.Module):
    """Causal TransformerEncoder (decoder-like) with n-conditioning."""
...
def train_one_step(model: nn.Module, opt: torch.optim.Optimizer, batch: Batch):
    ...
    loss = F.cross_entropy(
        ...
        ignore_index=SPECIAL["EOS"]   # <-- simple and safe
    )
    opt.zero_grad()
    loss.backward()
...
def run_patternboost(
    ...
    # interpret motif_mode
    use_motif_seed    = motif_mode in ("both", "seed")
    use_motif_refresh = motif_mode in ("both", "refresh")

    n_list = ns_sequence(n_start, n_target, lift_step)
    run_dir = make_run_dirs(out_root, seed, n_list)
    print(f"[device] {DEVICE}")
    print(f"[run_dir] {run_dir}")
    print(f"[motifs]  seeding={use_motif_seed}  refresh={use_motif_refresh}")
    ...
    # Initial seeds
    seeds = seeded_pool(
        n, rng, seeds_per_round,
        use_motif_seeds=use_motif_seed,
        use_corner_combo=use_motif_seed
    )
    ...
    while n <= n_target:
        ...
        for r in range(rounds_per_n):
            ...
            # 3) New seeds: recombine + transformer + elite-mutation + (optional motif refresh)
            new_seeds: List[Instance] = []
            # 3a) recombination among elites of same n
            new_seeds.extend(recombine_seeds(elites, k=max(1, seeds_per_round//4), rng=rng, n=n))

            # 3b) fill remaining with model samples / elite-mutants
            ...
            # 3c) (optional) inject a small motif refresh
            if use_motif_refresh:
                mix = []
                RH, RV = motif_corner_combo(n)
                mix.append((RH[:], RV[:]))
                mix.append((RV[:], RH[:]))
        ...
        # Curriculum lift to n + lift_step
        if elites:
            n_next = n + lift_step
            lifted=[]
...
def main():
    ...
    ap.add_argument("--motif_mode",
                    choices=["both", "seed", "refresh", "off"],
                    default="both",
                    help="Use motifs at seeding+refresh (both), seeding only (seed), "
                         "refresh only (refresh), or disable (off).")
